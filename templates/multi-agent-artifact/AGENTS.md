...
- **Backend**: <Framework + Language + Version> (예: Java Spring Boot 3.x, JDK 17)
- **Frontend**: <Framework + Runtime> (예: Expo React Native, Next.js)
- **Storage**: <DB + Object Storage> (예: PostgreSQL, R2)
- **Auth/Identity**: <Provider> (예: Supabase, Auth0, 자체 JWT)
- **외부 의존**: <주요 third-party API>
- **(Web3인 경우) Chain**: <Network + Toolchain> (예: Solana + Anchor, Base + Foundry)

---

## Conventions

<!-- 프로젝트별 컨벤션이 정해지면 여기 채운다 (네이밍, 라우팅, 응답 포맷 등) -->

---

## Commands

<!-- 자주 쓰는 빌드/테스트/배포 명령어 (./gradlew.bat test 등) -->
...
<!-- 자동 체크 명령 + 통과 기준. plan.md Sprint Contract에서 cycle별로 재명시. -->
...
| 역할 | 모델 | 호출 빈도 | 산출물 |
|------|------|----------|--------|
| Planner | Codex plan mode | cycle 시작 시 1회 | `selection.json` + `plan.md` |
...
- Planner는 `plan.md`와 함께 `.review/cycle-N/selection.json`을 작성한다. 기존 cycle에는
  소급 강제하지 않지만 신규 cycle은 남은 후보 2~5개, 선택 이유, 예측 시간·비용·first-pass
  확률을 기록한다. 템플릿은 `.review/_templates/selection-template.json`을 사용한다.
- Executor의 첫 동작은 현재 브랜치 확인 + 필요시 `git switch`.

### Cycle Selection Contract

Planner는 가장 쉬운 항목이 아니라 다음 구현 슬라이스로서 가장 적합한 항목을 선택한다.
...
- 각 점수는 1~5(`repetitionPenalty`, `fragmentationPenalty`는 0~5)이며 spec ID, 선행관계, 현재 코드 경로 등
  근거를 최소 1개 기록한다.
- 숫자는 절대적 진실이 아니라 선택 당시의 예측이다. 완료 후 metrics가 실제 시간·비용·
  first-pass와 비교하므로 결과를 보고 과거 `selection.json`을 고치지 않는다.
- 동일한 정적/문서/표면 패턴이 반복되면 개별 easy cycle로 계속 선택하지 말고 독립 검증
  가능한 범위에서 묶는다.
- 같은 FR의 task를 하나만 선택하고 동일 FR 후보를 남기면 `fragmentation.splitRationale`에
  분리로 줄어드는 리스크와 추가 반복 테스트 시간·고정 검증비를 비교한다. 이 근거가 없으면
  `/implement` gate가 cycle을 거부한다.
- 동일 FR 후보가 없어도 단일-task 후보의 예상 test/implementation 시간이 35% 이상이거나
  고정 검증비/총비용이 25% 이상이면 같은 분할 근거가 필요하다.
- 가장 높은 total을 선택하지 않았다면 `decision`에 리스크, 의존성 또는 사용자 판단의 이유를
  명시한다.
- `scoreVersion`과 weights는 최소 20개 완료 cycle 동안 유지한 뒤 meta-cycle에서 조정한다.
