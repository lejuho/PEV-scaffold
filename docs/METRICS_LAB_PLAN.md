...
## 10. v2 측정 계약 — 선택 품질과 비용 구간

- 신규 cycle의 Planner 산출물은 `selection.json` + `plan.md`다. 선택 파일은 후보
  2~5개, scoreVersion/weights, 근거, duration/test/cost/first-pass 예측을 구현 전에
  고정한다.
- 비용은 `implementation`, `verification`, `rework`, `merge`로 분해한다.
  `rework`는 pass-001 완료 뒤가 아니라 첫 `BLOCKED` verdict 뒤부터다.
- cycle/event 시각이 파일 artifact보다 우선한다. 뒤늦게 생성된 metric event가 과거
  done 시각보다 늦으면 artifact fallback을 사용하고 `dataQuality`에 남긴다.
- 누적 hands-off 시간은 event start와 verdict가 모두 있는 cycle만 합산한다. mtime
  backfill 시간은 `backfilledElapsedHours`로 별도 보존한다.
- meta-cycle은 selection coverage, low-value streak, duration/test/cost MAE와 first-pass
  Brier score를 함께 읽고 최소 20 cycle 단위로 rubric/weights를 조정한다.

## 11. v3 측정 계약 — 실행 시간과 정규화 단위

- runner는 agent turn 시작/종료를 `agent_turn_started`/`agent_turn_finished`로 남긴다.
  이 구간의 합집합이 `activeAgentTime`이며, wall clock인 `handsOffElapsed`와 분리한다.
- Telegram/dashboard 명령은 cycle, origin, intervention type을 직접 기록한다.
  조회(`observation`)는 구조 개입 수에서 제외하고 guidance/retry/manual progress/merge/
  override/flow control을 따로 집계한다.
- cycle 라벨이 있는 Git commit은 이벤트가 없는 과거 cycle의 timestamp fallback이자
  changed files/lines 분모다. mtime은 마지막 fallback일 뿐 역사적 사건 시각이 아니다.
- `Spec:`의 FR/SPEC ID와 선택 후보의 `includedTasks`를 이용해 requirement/task별 비용,
  lead time, cycle amplification을 계산한다. 한 cycle에 여러 단위가 있으면 비용은 균등
  배분해 총비용이 부풀지 않게 한다.
- executor check는 command/durationSec/exitCode 구조로 기록한다. legacy 문자열은
  `unknownOutcome`으로 남기며 성공 0건으로 간주하지 않는다.

## 12. Fragmentation 억제 계약

- selection v2는 `fragmentationPenalty`, 반복 테스트 예상 시간, 고정 검증비를 후보별로
  기록한다.
- 같은 FR의 후보가 남아 있는데 단일 task cycle을 선택하면 20자 이상의
  `fragmentation.splitRationale`가 필수다. 같은 FR 후보가 없더라도 예상 테스트 시간이
  구현 시간의 35% 이상이거나 고정 검증비가 총비용의 25% 이상인 단일-task cycle에도
  같은 gate를 적용한다. `/implement` 전에 검사한다.
- requirement가 여러 cycle에 걸치면 동일 check command가 뒤 cycle에서 소비한 시간을
  `repeatedCheckOverheadSec`로, 첫 cycle 이후 verification 비용을
  `additionalCycleVerificationCostUsd`로 집계한다.
- `amplifiedRequirements`는 관찰값이고 `fragmentedRequirements`는 실제 반복 check 또는
  추가 검증비가 관찰된 부분집합이다. check coverage가 없으면 fragmentation 0이 아니라
  비용 미관측으로 해석한다.
